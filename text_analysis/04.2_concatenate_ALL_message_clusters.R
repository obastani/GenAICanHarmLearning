...
# Get all files in arm_question
file_list = list.files('data/all_message_cluster_by_arm_grade')
...
for (filename in file_list){
  ...
  data = read.csv(paste0("data/all_message_cluster_by_arm_grade/", filename))
  
  # ===================================================
  # Create topic - representative_doc mapping
  # ===================================================
  # Note it is repdoc, not Representative_Docs
  ...
  # Meta clustering
  pairwise_comparison = matrix(nrow = nrow(result), ncol = nrow(result))
...
# Save
write.csv(total_df, file = "results/top_messages_by_arm_question_s1_4_all_messages_final.csv")
